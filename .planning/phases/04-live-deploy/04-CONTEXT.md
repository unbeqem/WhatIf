...
- **D-09:** Impressum page is scaffolded with clearly-marked placeholders for the legally-required personal details (name, address) the founder must fill in; contact = `hello@whatif.app` (already the Footer contact). Datenschutz can be written substantially complete describing the actual processors: Supabase (EU, auth+DB), Stripe (payments), OpenAI (decision text sent to model, not persisted to a profile), Resend (auth emails), plus the IP/user rate-limit + abuse logging — controller/contact details as placeholders. Use existing globals.css tokens + section rhythm; keep copy English to match the rest of the UI (note: a DE Impressum/Datenschutz is conventionally in German — planner to confirm language, but user-facing product copy is English per CLAUDE.md).
